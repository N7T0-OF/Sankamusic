...
# ---------------------------------------------------------------------------
FEATURES=$(cat <<'EOF'
nav-custom|Navigation|Barre de navigation personnalisable (styles, sections, ordre, raccourcis)|customNavigation|customNavigation|SpaceKaiFeatures::customNavigation|Sélection exclusive de style (verre liquide / translucide / minimaliste) + ordre/masquage des sections persistant (NavCustomization/SpaceKaiNavCustomizer), câblé (audit-features PASS). Raccourcis d'actions non livrés. Statique — runtime à confirmer. Voir scripts/audit-navigation.sh.|
nav-minimal|Navigation|Style de barre minimaliste|minimalisticNavigation|minimalisticNavigation|SpaceKaiFeatures::minimalisticNavigation|Style minimaliste réel : icônes seules (texte masqué via showItemLabels) + hauteur réduite, sélection exclusive avec les autres styles. Statique — runtime à confirmer. Voir scripts/audit-navigation.sh.|
...
dynamic-color|Thème|Dynamic Color Android (fond + surfaces)|dynamicColor|dynamicColor|SpaceKaiFeatures::dynamicColor|Flag ON → palette Material You unpinned (platformDynamicColorSchemeUnpinned) ; aucune surface de thème normal ne force le noir (barres de sélection → surfaceContainerHigh). Noirs restants = par design (écrans immersifs ForceDarkContent, scrims, overlays). Voir scripts/audit-dynamic-color.sh. Device run NON VÉRIFIÉ.|
...
haptics|Vibration|Vibration (événements, intensité)|haptics|haptics|HapticsSpaceKai|HapticManager centralisé (ON/OFF + intensité Faible/Moyenne/Forte) câblé sur la sélection d'onglets (barre plate/rail/verre liquide), les toggles SpaceKai, le slider crossfade et les actions player. Statique — ressenti/runtime à confirmer. Voir scripts/audit-features.sh.|features/haptics/HapticsSpaceKai.kt
crossfade-slider|Audio|Fondu enchaîné : slider visuel|-|crossfadeSliderSec|CROSSFADE_DURATION_AUTO|Slider visuel 1-30s (0 = Auto) dans SettingScreen, valeur affichée en temps réel, haptic au relâchement gated par le flag haptics. Statique — runtime à confirmer.|
...
updater|Mise à jour|Mise à jour interne (check + téléchargement + installation)|-|SpaceKaiUpdatesSection|SpaceKaiUpdateManager|Détection OK + téléchargement APK universel + SHA-256 avant installation + vérification du package name (getPackageArchiveInfo) + Package Installer Android (expect/actual PlatformUpdater). Détection=CORE, download/install=composeApp. Vérifié compile+jvm+gate+CI Android; device run NON VÉRIFIÉ. Voir scripts/audit-updater-flow.sh.|spacekai/update/SpaceKaiUpdateManager.kt
