...
Ground truth: the code reviewed on tips `37bf53ae` (parent) / `959c580` (core).
Scope: every claim below is observable on one Android device; nothing here can be
proven by the JVM suite (44/44 covers logic only, not the install pipeline).
...
| P3 | A published GitHub release on `N7T0-OF/Sankamusic` tagged `v0.3.7` with exactly the assets the code resolves: a **universal release APK** (e.g. `sankamusic-v0.3.7-universal-release.apk`) uploaded **first**, and `SHA256SUMS.txt` (case-insensitive filename match) | `gh release view v0.3.7 --json assets` ; asset order matters — `resolveApkUrl` picks the **first** `.apk` whose name contains none of `-debug`, `-unsigned`, `-signed-`, `-test`, `.breakpoints`, `-arm64-v8a`, `-armeabi-v7a`, `-x86_64`, `-armv7a`, `-arm64` |
| P4 | `SHA256SUMS.txt` format: one line per asset, `<sha256-hex>  <filename>.apk` (hash first, filename last). The verifier matches **by hash only** — any line whose leading hex equals the downloaded APK's digest and ends in `.apk` | `sha256sum sankamusic-v0.3.7-universal-release.apk` matches the manifest line |
| P5 | For beta tests: a published **prerelease** tagged `v0.3.8-beta.1` with the same asset shape | `gh release view v0.3.8-beta.1` |
...
### T2 — Upstream compatibility: 2.0 → 2.1 → future (info-only, no install offer)

1. Latest upstream = `v2.0.0` → label `✓ À jour avec la dernière release officielle` (tested base).
2. Upstream publishes `v2.1.0` (declared, untested base) → label `⚠ Nouvelle release officielle (v2.1.0) — base supportée, validation appareil en cours`. **No** upstream install button anywhere (block is info-only by design).
3. Upstream publishes `v2.2.0` (undeclared) → label `⚠ Nouvelle release officielle détectée — SpaceKai pas encore compatible`; the only offer is the upstream repo page.

**PASS if:** the three labels render exactly as above and no upstream APK is ever offered for install. Simulate without waiting: edit `UpstreamCompatibilityMatrix.bases` locally if needed — but re-build and re-sign before treating the test as done.

### T3 — Stable user is NEVER offered a prerelease

Fixture: publish **only** `v0.3.8-beta.1` as a prerelease (no newer stable).
...
**PASS if:** **no** update dialog and **no** update row (both gates are `isVersionNewer`); logcat shows no resolved prerelease URL. **FAIL if** any beta tag/URL appears in UI or logcat.

### T4 — Beta user gets the NEWEST prerelease

1. Channel **Beta** (Settings → SpaceKai Updates → Canal SpaceKai). Installed 0.3.7.
2. Published state: stable `v0.3.7`, prereleases `v0.3.8-beta.1` (newest). Check for updates.
3. `selectSpaceKaiUpdate` filters candidates newer than installed, max by semver → `v0.3.8-beta.1` offered.

**PASS if:** dialog/row shows `v0.3.8-beta.1`; install succeeds with the same pipeline as T1.
...
### T5 — Negative: release WITHOUT SHA256SUMS.txt refuses

Fixture: publish `v0.3.7` with the APK only, no checksums file.
...
### T6 — Negative: CORRUPTED SHA256SUMS.txt refuses

Fixture: publish `SHA256SUMS.txt` with a wrong hash for the APK.
...
### T7 — Negative: TAMPERED APK refuses

Fixture: correct `SHA256SUMS.txt`, but flip one byte in the published APK (re-upload, keep the original hash in the manifest).

**PASS if:** same as T6 — hash mismatch → refusal, file deleted, no installer prompt.

### T8 — Negative: no debug/unsigned fallback

Fixture: publish `v0.3.7` carrying **only** `sankamusic-v0.3.7-debug.apk` (or any asset matching an excluded token).

**PASS if:** `resolveApkUrl` returns null → row/dialog shows no install action; tapping the update row results in `Aucun APK trouvé dans la release.` (FAILED) or the releases-page fallback — never a download of the debug APK. `adb logcat` must contain no `-debug` URL.

### T9 — Optional (hardening): package-name mismatch refuses

Fixture: a release whose APK is a **different** app (package name ≠ `com.maxrave.simpmusic`), correctly hashed.
...
- T3/T4 fixtures must be re-published (prerelease→draft, or delete) so a later Stable test can't be polluted by a leftover beta.
...
All of T1–T8 **PASS** (T9/T10 recommended, not blocking), evidence kit attached → the merge gate
on Sankamusic#5 / core#1 is lifted. Any deviation from the expected strings/phases above is a
bug report against the updater, not a pass.